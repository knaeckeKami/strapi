...
const TrialCountdown = () => {
  ...
  const { data } = useGetLicenseTrialTimeLeftQuery(undefined, {
    ...
    <Flex justifyContent="center" padding={3}>
      <Tooltip
        label={formatMessage(
          ...
          {
            date: data?.data.trialEndsAt ? format(new Date(data.data.trialEndsAt), 'PPP') : '',
          }
        )}
        side="right"
      >
        <div>
          <CircleProgressBar percentage={((30 - (data?.data.daysLeft || 0)) * 100) / 30} />
        </div>
      </Tooltip>
    </Flex>
  );
